...
int c_list_drives(C_DriveInfo* out_drives, int max_drives) {
    ...
    while (*drive && count < max_drives) {
        ...
        // Filter out CD-ROMs, network drives, etc. Focus on FIXED and REMOVABLE.
        if (type == DRIVE_FIXED || type == DRIVE_REMOVABLE) {
            C_DriveInfo info;
            memset(&info, 0, sizeof(C_DriveInfo));

            // Format volume path for CreateFile, e.g. "\\.\C:"
            // drive has 3 characters: "C:\"
            char drive_letter = drive[0];
            snprintf(info.path, sizeof(info.path), "\\\\.\\%c:", drive_letter);

            // Get volume name and filesystem type
            ...
            if (type == DRIVE_REMOVABLE) {
                ...
                // Default to SSD for modern resume vibe, or HDD
                strncpy(info.type, "SSD", sizeof(info.type) - 1);
            }

            // Primary drive indicator (usually C:)
        ...
        // Advance to next drive string (null-terminated series ending in double null)
